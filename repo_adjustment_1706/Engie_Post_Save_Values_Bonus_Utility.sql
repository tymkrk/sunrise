CREATE PROCEDURE [dbo].[Engie_Post_Save_Values_Bonus_Utility]
@kMValuesData AS [dbo].[Kernel_Type_k_m_values] READONLY
AS
/*
	Author: Rafal Jura
	Date: 2022-02-28
	Description: procedure refreshes values in Bonus Utilities Campaign

	Changes:
	(1) 12-10-2022 AB - Logic for MRT Flag
	(2) 03-11-2022 Joanna Rokosz: Max Exceeded logic change
	...
	(10) 18-03-2024 - Mateusz Paluch - ZD#104811 - getting rid of multiplication by 100 for BonusRangeMaximum in calculating bonus exceeded message, 
		changed value to which it is compared to(to proposed)  and changed "<" sign to "<="
	(11) - 22-05-2024 - Tymek Kruk - Bonus in Employee Currency should be round up
	(12) 03-06-2024 - Kamil Roganowicz: added Current_ASF refresh
	(13) 14-06-2024 - Kamil Roganowicz: ENG-89 change denominator in GF_BUT_Pourcentage_Bonus_Proposal calculation
	(14) 04-07-2024 - Rafal Jura: ENG-117  % Bonus (Excl. Exceptional Bonus), % Bonus (Excl. Exceptional Bonus), Bonus Amount in Employee Currency (Excl. Exceptional Bonus) should be NULL by default not 0
	(15) 05-08-2024 - Rafal Jura: ENG-167 fixed calculations of % Bonus (Incl. Exceptional Bonus) - changed to Base Salary without Prorata
	(16) 12-09-2024 - Rafal Jura: ENG-218 fixed calculations of % Bonus (Excl. Exceptional Bonus) - use base Salary without Prorata and Current ASF
	(17) 06-02-2025 - Kamil Roganowicz: ENG-296 changed logic for MRT and Deferred Flag
*/
BEGIN
	...
	BEGIN TRY
		...
		DECLARE @id_plan int = (SELECT id_plan FROM k_m_plans WHERE name_plan LIKE '%Bonus Utilities')
		DECLARE @id_yearly_campaign int = (SELECT ID FROM Engie_Param_Yearly_Campaign WHERE ID_Status IN (2,3))

		DECLARE @id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus int = (SELECT id_ind FROM k_m_indicators WHERE name_ind = 'GF_BUT_Yearly_Campaign_BUT_Bonus')
		DECLARE @id_ind_GF_BUT_Yearly_Campaign_BUT_Salary int = (SELECT id_ind FROM k_m_indicators WHERE name_ind = 'GF_BUT_Yearly_Campaign_BUT_Salary')
		DECLARE @id_ind_GF_BUT_MRT_Flag int = (SELECT id_ind FROM k_m_indicators WHERE name_ind = 'GF_BUT_MRT_Flag')


		DECLARE @id_field_GF_BUT_Bonus_Final_To_Be_Paid int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_Bonus_Final_To_Be_Paid')
		DECLARE @id_field_GF_BUT_Bonus_Amount_EUR int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_Bonus_Amount_EUR')
		DECLARE @id_field_GF_BUT_Bonus_Amount_EUR_2 int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_Bonus_Amount_EUR_2')
		DECLARE @id_field_GF_BUT_Salary_Prorata_EC int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_Salary_Prorata_EC')
		DECLARE @id_field_GF_BUT_Salary_Prorata int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_Salary_Prorata')
		DECLARE @id_field_GF_BUT_Bonus_Amount_Proposal int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_Bonus_Amount_Proposal')
		DECLARE @id_field_GF_BUT_Pourcentage_Bonus int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_Pourcentage_Bonus')
		DECLARE @id_field_GF_BUT_Bonus_Amount int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_Bonus_Amount')
		DECLARE @id_field_GF_BUT_ExceptionalBonus int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_ExceptionalBonus')
		DECLARE @id_field_GF_BUT_ExceptionalBonus_EUR int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_ExceptionalBonus_EUR')
		DECLARE @id_field_GF_BUT_MRT_Flag int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_MRT_Flag')
		DECLARE @id_field_GF_BUT_Deferral_Flag int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_Deferral_Flag')
		DECLARE @id_field_GF_BUT_Bonus_Max_Exceeded int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_Bonus_Max_Exceeded')
		DECLARE @id_field_GF_BUT_Pourcentage_Bonus_Proposal int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_Pourcentage_Bonus_Proposal')
		DECLARE @id_field_GF_BUT_New_ASF int = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_New_ASF')
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus AS id_ind,
			@id_field_GF_BUT_Bonus_Amount_Proposal AS id_field,
			CASE
			...
			'Engie_Post_Save_Values_Bonus_Utility',
			1,
			0
		FROM Engie_Cache_View_Process_Bonus_Utility c
		...
		INNER JOIN k_m_values v_am
			...
			AND v_am.id_ind = @id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus
			AND v_am.id_field =  @id_field_GF_BUT_Bonus_Amount_Proposal
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus AS id_ind,
			@id_field_GF_BUT_ExceptionalBonus AS id_field,
		...
		INNER JOIN k_m_values v_am
			ON v_am.id_step = ps.id_step
			AND v_am.id_ind = @id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus
			AND v_am.id_field = @id_field_GF_BUT_ExceptionalBonus
		WHERE c.ID_YearlyCampaign = @id_yearly_campaign
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus AS id_ind,
			@id_field_GF_BUT_ExceptionalBonus_EUR AS id_field,
	...
	*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus AS id_ind,
			@id_field_GF_BUT_Bonus_Amount_EUR_2 AS id_field,
		...
		INNER JOIN k_m_values v_am
			ON v_am.id_step = ps.id_step
			AND v_am.id_ind = @id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus
			AND v_am.id_field = @id_field_GF_BUT_Bonus_Amount_EUR
		LEFT JOIN k_m_values v_am_2
			ON v_am_2.id_step = ps.id_step
			AND v_am_2.id_ind = @id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus
			AND v_am_2.id_field =  @id_field_GF_BUT_ExceptionalBonus_EUR
		WHERE c.ID_YearlyCampaign = @id_yearly_campaign
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_Yearly_Campaign_BUT_Salary AS id_ind,
			@id_field_GF_BUT_Salary_Prorata_EC AS id_field,
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_Yearly_Campaign_BUT_Salary AS id_ind,
			@id_field_GF_BUT_Salary_Prorata AS id_field,
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus AS id_ind,
			@id_field_GF_BUT_Bonus_Final_To_Be_Paid AS id_field,
			v_ba.input_value,
			v_ba.input_value_numeric,
			GETUTCDATE() AS input_date,
			-1 AS id_user,
			'Automatically Insert' AS comment_value,
			'Engie_Post_Save_Values_Bonus_Utility' AS source_value,
			1 AS value_type,
			0 AS idSim
		FROM k_m_plans_payees_steps ps
		INNER JOIN #temp_steps t
		...
		LEFT JOIN k_m_values v_ba
			...
			AND v_ba.id_ind = @id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus
			AND v_ba.id_field = @id_field_GF_BUT_Bonus_Amount_Proposal
		WHERE ps.id_plan = @id_plan
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus AS id_ind,
			@id_field_GF_BUT_Bonus_Amount_EUR AS id_field,
		...
		LEFT JOIN k_m_values v_cbs_ec
			...
			AND v_cbs_ec.id_ind = @id_ind_GF_BUT_Yearly_Campaign_BUT_Salary
			AND v_cbs_ec.id_field = @id_field_GF_BUT_Salary_Prorata_EC
		LEFT JOIN k_m_values v_cbs_e
			ON v_cbs_e.id_step = ps.id_step
			AND v_cbs_e.id_ind = @id_ind_GF_BUT_Yearly_Campaign_BUT_Salary
			AND v_cbs_e.id_field = @id_field_GF_BUT_Salary_Prorata
		WHERE ps.id_plan = @id_plan
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus AS id_ind,
			@id_field_GF_BUT_Pourcentage_Bonus AS id_field,
			CASE
		...
		LEFT JOIN k_m_values v_ba_exc
			...
			AND v_ba_exc.id_ind = @id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus
			AND v_ba_exc.id_field = @id_field_GF_BUT_ExceptionalBonus
		LEFT JOIN k_m_values v_cbs_ec
			ON v_cbs_ec.id_step = ps.id_step
			AND v_cbs_ec.id_ind = @id_ind_GF_BUT_Yearly_Campaign_BUT_Salary
			AND v_cbs_ec.id_field = @id_field_GF_BUT_Salary_Prorata_EC
		INNER JOIN Engie_Cache_View_Process_Bonus_Utility ecvpbc
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus AS id_ind,
			@id_field_GF_BUT_Pourcentage_Bonus_Proposal AS id_field,
			CASE
			...
			END AS input_value,
			(COALESCE(v_ba.input_value_numeric, 0)) / NULLIF(ISNULL(c.BaseSalary,0) + ISNULL(c.Current_ASF,0), 0) AS input_value_numeric,
			GETUTCDATE() AS input_date,
			-1 AS id_user,
			'Automatically Insert' AS comment_value,
			'Engie_Post_Save_Values_Bonus_Utility' AS source_value,
		...
		INNER JOIN #temp_steps t
			ON t.id_step = ps.id_step
		INNER JOIN Engie_Cache_View_Process_Bonus_Utility c
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus AS id_ind,
			@id_field_GF_BUT_Bonus_Amount AS id_field,
			CASE
				WHEN v_ba.input_value_numeric IS NULL AND NULLIF(v_ba_exc.input_value_numeric, 0) IS NULL
				THEN NULL
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_MRT_Flag AS id_ind,
			@id_field_GF_BUT_MRT_Flag AS id_field,
			CASE WHEN 
			[dbo].[_fn_mrt_flag](ps.id_payee, 'Utilities') != '' 
			THEN 
			[dbo].[_fn_mrt_flag](ps.id_payee, 'Utilities')
			ELSE 
			NULL
	...
	/*	INNER JOIN k_m_values v_am
			ON v_am.id_step = ps.id_step
			AND v_am.id_ind = (SELECT id_ind FROM k_m_indicators WHERE name_ind =  'GF_BUT_Yearly_Campaign_BUT_Bonus')
			AND v_am.id_field = (SELECT id_field FROM k_m_fields WHERE code_field = 'GF_BUT_Bonus_Amount')*/
		WHERE c.ID_YearlyCampaign = @id_yearly_campaign
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_MRT_Flag AS id_ind,
			@id_field_GF_BUT_Deferral_Flag AS id_field,
			CASE
				...
				WHEN (v_mrt.input_value = 'MRT - Ex-post' OR v_mrt.input_value = 'MRT - Ex-ante') AND 
					(COALESCE(v_bon.input_value_numeric, v_bon.input_value,0)>50000 OR COALESCE(v_bon.input_value_numeric, v_bon.input_value,0) > 0.25*(c.BaseSalary+COALESCE(v_bon.input_value_numeric, v_bon.input_value,0))) THEN 'Bonus deferred, refer to Bonus Scheme Report'		
			END AS input_value,
			GETUTCDATE(),
			-1 AS id_user,
			'Automatically Insert',
		...
		INNER JOIN k_m_values v_bon
			...
			AND v_bon.id_ind = @id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus
			AND v_bon.id_field = @id_field_GF_BUT_Bonus_Final_To_Be_Paid 
		LEFT JOIN k_m_values v_mrt
			ON v_mrt.id_step = ps.id_step
			AND v_mrt.id_ind = @id_ind_GF_BUT_MRT_Flag
			AND v_mrt.id_field = @id_field_GF_BUT_MRT_Flag
		LEFT JOIN (SELECT
					exR.exchangeRatefor1EUR,
					exR.currency,
					exr.EffectiveDate,
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus AS id_ind,
			@id_field_GF_BUT_Bonus_Max_Exceeded AS id_field,
			CASE
				WHEN v_pct.input_value_numeric IS NULL THEN NULL
				WHEN COALESCE(c.BonusRangeMaximum, 0) <= v_pct.input_value_numeric --(10)
		...
		INNER JOIN k_m_values v_pct
			...
			AND v_pct.id_ind = @id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus
			AND v_pct.id_field = @id_field_GF_BUT_Pourcentage_Bonus_Proposal
		WHERE c.ID_YearlyCampaign = @id_yearly_campaign
		...
		*/
		...
		SELECT
			ps.id_step,
			@id_ind_GF_BUT_Yearly_Campaign_BUT_Bonus AS id_ind,
			@id_field_GF_BUT_New_ASF AS id_field,
